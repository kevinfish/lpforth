...
\ save as mkernel.f for a meta compiled version, so ll will work
\ add min-time to modify input style change words to accomodate 
\ save as mkernel2.f 12/6/98
\ reverse init-libs and init-procs so program can stand alone
\ 1/19/99 save as mkernel3.f
\ save as mkernel4.f before strip down 2/4/99
\ save as mkernel5.f before unify word list 2/6/99
\ save as mkernel6.f before unify procedure to word list 2/7/99
\ save as mkernel7.f before get rif of all the ncall 2/7/99
\ thread version ok, save as mkernel8.f 2/12/99
\ save as mkernel9.f before redifine compare to win32for style Sat Mar 25 16:24:45 2000
\ ************* procedure to make a new system ************
\ run cmeta to make a new base system, modify size if necessary. lpforth.bin will be generated
\ run lpforth, fload meta.f, it will generate lpforth.new
\ run lpforth.new, fload mkernel.f, then fsave lpforth.temp
\ cp lpforth.temp lpforth -f
\ save as mkernel10.f before replace i j k with low level ones
\ save as mkernel11.f for soft link Wed Apr 26 00:41:22 2000
\ save as mkernel12.f to try low level i j k
\ mkernel13.f for low level cmove> and cmove
\ save as mkernel14.f to fix +to Mon May 1 13:01:57 2000
\ fix leave, save as mkernel15.f Tue Mar 6 22:32:23 2001
\ change (+loop) to low level Wed Mar 7 00:33:28 2001
...
7 constant fwrite-x
 8    constant dlopen-x
 9    constant dlsym-x
10    constant dlclose-x
13    constant dlerror-x
...
: link>thread-no ( link -- thread-no-adr )
	cell+ ;

: link>table ( link -- thread-no-adr )
	2 cells+ ;
...
: >name ( cfa -- nfa | 0 ) \ string length at both ends
	dup sys-base here between
	if dup dup 1- c@ 2 + - dup name> rot = not
		if drop false
		then
	else drop false
	then
...
: asee  ( adr -- )
        ...
        1+
        begin cr cell+ dup space .adr dup @ >name dup . dup 
                if .id
                else drop dup @ u.
                then nuf?
        until drop ;
...
: simage ( -- )
        ...
        "wb" pad 1+ fopen-x xcall file-h ! drop drop
        file-h @ here sys-base - 1 sys-base fwrite-x xcall . drop drop drop drop
        file-h @ fclose-x xcall . drop ;

\ v12 {
\ file word format
\       cell    file number
\ runtime : return file number


\ change to regular word list
...
: #vocabulary ( n -- )
	create voc-link link, 
		dup , cells here over 0 fill allot
	does> context ! ;
...
0 file-link ! \ file-link keep filev voc adr

: link>content ( link-adr -- content-adr )
	cell+ ;

: link>fname ( link-adr -- name-adr )
	2 cells+ ;
...
: file ( n | name -- )
	create file-link link,
		, 
		last @ count here over 2 + allot place0
	does> link>content @ ;
...
: .files ( -- )
	file-link
        begin   @ dup nuf? 0 = and
        while   dup link>fname .id
                        ." [" dup link>content @ . ." ]" space
        repeat  drop cr ;
...
: to  ( n -<value_name>- ) \ make it compatible for extraction
       	' >body cell+
        state @ if compile dovalue , compile ! else ! then ; immediate
...
: +to ( n -<value_name>- )
        ' >body cell+
        state @ if compile dovalue , compile (+to) else (+to) then ; immediate
...
: forget-a-thread ( lfa thread-adr -- )
        dup>r 
        begin @ dup r@ !
        	2dup >
        until 2drop r>drop ;
...
: empty-voc? ( voc -- f )        
	...
	do dup link>table i cells+ @ 0<>
		if nip false swap leave ( false voc )
		then
	loop drop ;
		
: forget-a-voc ( lfa voc -- )
	dup link>thread-no @ 0
	do 2dup link>table i cells+ forget-a-thread
	loop nip dup empty-voc?
	if delete-voc
	else drop
	then ;
...
: hide          ( -- ) \ last contain last nfa
                last @ cell- @ last @ count current @ thread ! ;
...
\ solib.f use vocabulary to replace special link
\ **********************************************************************
\
\ library format:
\ 	cell 	link
...
\	cell 	no of parameters
\		name
\ **********************************************************************

marker lib

: link>lname ( link-adr -- library-name-adr )
	3 cells+ ;

: link>pname ( link-adr -- proc-name-adr )
	4 cells+ ;

: link>lib ( link-adr -- lib-adr )
	2 cells+ ;

: content>lname ( lib -- content-adr )
	2 cells+ ;

: proc>para ( proc-adr -- para-adr )
	2 cells+ ;
				
	

variable lib-link            \ save the library voc addr
0 lib-link !

variable proc-link           \ save the procedure voc addr
0 proc-link !
 
: load-library  ( lib -- err? )        \ f1=FALSE if all is ok
        dup @ 0=                        \ library address NULL?
        if      dup content>lname           \ step from PFA to counted library name
                1+ 1 256 or swap dlopen  ( LoadLibrary ) \ RTLD_LAZY RTLD_GLOBAL or 
                dup>r r@
...
: library-exist? ( z" -- f )
        256 1 or swap dlopen 0= not ;

: library ( | lib$ 'name.so' -- )
	>in @ 
	bl word count lib$ place0
	>in !
        lib$ 1+ library-exist? not if dlerror . ." Cannot find library."  then \ abort" Cannot find library."
	create  lib-link link,
		256 1 or lib$ 1+ dlopen , \ the library handle, lazy and global
            	0 ,                \ mark as application library
            	lib$ count here over 2+ allot place0
	does> ; ( -- adr )
	
: load-proc ( proc -- err? )
        dup @ 0=                        \ proc address NULL?
        if      dup link>content @ load-library 0=
                if dup content>lname 1+ over link>lname @ @ dlsym swap ! false
                else drop true
                then
...
: exist-in-lib? ( -- false | lib-adr ) \ string in proc-name
        lib-link
        begin   @ ?dup
        while   proc-name 1+ over link>content @ dlsym
                if      link>content  
                        EXIT                \ got it
                then
        repeat  \ no found
...
: ?proc-error   ( f1 -- )       \ f1=error if true
                if      proc-name count pocket place0
                	proc-name count type ascii ? emit 
...
\ n s" proc-name" proc forth-name                                                                                                
: proc ( n adr len -- ) \ try to define proc if can
        proc-name place0   \ get the proc name
        exist-in-lib? dup 0= 
        if drop true abort" proc not found"
        then ( n lib-adr )
        create proc-link link,
        	proc-name 1+ over @ dlsym , , , 
        	proc-name count here over 2+ allot place0
        does> ( [args] adr -- )
        	link>content dup>r @ call ( [args] ret )
        	r> proc>para @ swap >r 
        	ndrop r> ;
...
: .libs         ( -- )
                ...
                begin   @ dup
                        nuf? 0= and
                while   dup link>lname count type space 
                        ." [" dup link>content @ . ." ]" space
                repeat  drop cr ;
...
: .procs        ( -- )
                ...
                begin   @ dup
                        nuf? 0= and
                while   dup link>pname count type space
                        ." [" dup link>content @ . ." ]" space
                repeat  drop cr ;
...
: init-libs ( -- )
		0 
                ...
                repeat drop
                begin ?dup 
                while   256 1 or over link>lname count lib$ place0 lib$ 1+ dlopen 
                        swap link>content !
                repeat ;
...
: init-procs ( -- )
		0 
                ...
                repeat drop
                begin  ?dup
                while   dup link>pname count proc-name place0 
                	proc-name 1+ over link>lib @ @ dlsym
                        swap link>content ! 
                repeat ;
...
#thread @ #vocabulary forth
' forth 9 + #thread @ 2+ cells cmove \ replace the old forth with new one
...
\ library libc.so
library libm.so \ glibc put the following procs in libm.so
\ it is not necessary to open libc.so


2 s" tcgetattr" proc tcgetattr
3 s" tcsetattr" proc tcsetattr

create save-termios 80 allot

: non-canon ( -- )
      pad 1 tcgetattr abort" tcgetattr error"
      pad save-termios 80 cmove
      1 pad 22 + c!
      0 pad 23 + c!
      $1 $2 $8 $8000 or or or not
      pad $3 cells + dup >r @ and r> ! 
      pad 2 1 tcsetattr abort" tcsetattr error" ;

non-canon

: min-time ( min time -- ) \ manipulate input style
      pad 1 tcgetattr abort" tcgetattr error"
      pad 22 + c!
      pad 23 + c!
      pad 2 1 tcsetattr abort" tcsetattr error" ;
...
: reset-termios ( -- )
      save-termios 2 1 tcsetattr abort" reset error" ;

: byebye bye ; \ low level bye

: bye reset-termios bye ;

1 s" fflush" proc fflush
...
create args-pad maxstring allot

: run-args ( -- )
	s" " args-pad place
	argc @ 1
	?do argv @ i cells+ @ asciiz->asc-len args-pad +place s"  " args-pad +place
	loop 
	args-pad count dup #tib ! tib swap cmove >in off
	interp
	;
...
#thread @ constant max-thread

create temp-link-table max-thread cells allot

0 value biggest

: get-bigest ( voc -- lfa | 0 )
 	-1 swap 0 to biggest
	link>thread-no @ 0
	do temp-link-table i cells+ @ dup biggest >
		if to biggest drop i 
		else drop
		then
	loop ( n )
	dup -1 =
	if drop 0
	else cells temp-link-table + dup @ >r dup @ @ swap ! r>
	then
	;
...
: voc-words ( voc -- )
	temp-link-table max-thread cells erase
	dup link>table over link>thread-no @ cells temp-link-table swap cmove
	cr horizontal-line
	cr dup 2 cells- 1- >name count type ."  vocabulary"  
	horizontal-line cr ( voc )
        begin dup get-bigest dup nuf? not and
        while link>n ?.id 
        repeat 2drop ;
...
: dump ( b u -- )
        ...
        over du-head
        16 / 1+ 0
        ?do cr 16 2dup dm+ rot rot 2 spaces _type
        loop drop r> base ! ;
...
4 s" fwrite" proc  fwrite
4 s" fread" proc  fread
3 s" fseek" proc  fseek
3 s" fgets" proc  fgets
\ set-proc fputs
1 s" strlen" proc  strlen
1 s" ftell" proc  ftell
1 s" rewind" proc  rewind
1 s" remove" proc  remove
2 s" rename" proc  rename
...
: read-file ( adr len fileid -- len ior )
        -rot dup>r swap 1 swap  fread dup r> <> ;

\ not conforming to ans standard

: reposition-file ( n fileid -- ior )
        seek-set -rot  fseek ;

: DELETE-FILE ( c-addr u -- ior )
         pocket place0 pocket 1+  remove ;

: FILE-POSITION ( fileid -- n ior )
         ftell dup -1 = ;

: FILE-SIZE ( fileid -- n ior )
        >r
        r@ file-position drop 
        seek-end 0 r@  fseek drop
        r@ file-position 
...
: to-tib ( adr len -- )
	dup 1+ #tib !
	tib place 10 tib count + c!
	0 >in ! ;
	
: set-file-link ( adr len -- )
	to-tib
	file-link @ link>content @ 1+ 
	dup loadfile !
	file ;
...
: write-file ( c-addr u fileid -- ior )
        -rot dup>r swap 1 swap  fwrite r> <> ;
        
: WRITE-LINE ( c-addr u fileid -- ior )
        >r 
	pocket place0 $a pocket count + c!
        pocket count 1+ 
	r> write-file ;
...
: RENAME-FILE  ( c-addr1 u1 c-addr2 u2 -- ior ) 
        ...
        pocket 1+ cur-file 1+  rename ;
  
\ memory

cr .( Loading memory words. )

1 s" malloc" proc  malloc
1 s" free" proc  free
2 s" realloc" proc  realloc
1 s" system" proc  system
...
: unmask ( file-no -- file-no' )
	$3fff and ;
	
: .viewinfo     ( -<name>- line filename )
        ' >view @ $10000 /mod unmask loadfile ! to orig-loc 
                file-link
                begin   @ dup
                        0 = not
                while   dup link>content @ loadfile @ =
                        if link>fname ( nfa ) orig-loc swap exit
                        then
...
2 s" gettimeofday" proc gettimeofday
1 s" localtime" proc localtime
...
: time&date 
        timezone timeval gettimeofday abort" Error"
        timeval localtime to tm
        tm @ tm cell+ @ tm 2 cells+ @
        tm 3 cells+ @ tm 4 cells+ @ 1+ tm 5 cells+ @ 1900 + ;
        
1 s" usleep" proc usleep
...
\ fload lowlevel.f  \ still testing compatibility
